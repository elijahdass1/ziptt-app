...
export default async function DigitalOrdersPage() {
  ...
  return (
    <div style={{ background: 'var(--bg-primary)', minHeight: '100vh', color: 'var(--text-primary)', padding: '40px 24px' }}>
      <div style={{ maxWidth: '800px', margin: '0 auto' }}>
        ...
        {orders.length === 0 ? (
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
            {orders.map(order => (
              <div key={order.id} style={{ background: 'var(--bg-secondary)', border: '1px solid #1A1A1A', borderRadius: '12px', padding: '24px' }}>
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'start', marginBottom: '16px' }}>
                  <div>
                    <h3 style={{ fontSize: '16px', marginBottom: '4px' }}>{order.product.name}</h3>
                    <p style={{ fontSize: '13px', color: 'var(--text-secondary)' }}>{new Date(order.createdAt).toLocaleDateString('en-TT', { dateStyle: 'medium' })}</p>
                  </div>
                  <div style={{ textAlign: 'right' }}>
                    <span style={{ background: order.status === 'delivered' ? '#052e16' : '#1c1917', color: order.status === 'delivered' ? '#4ade80' : 'var(--text-secondary)', padding: '4px 12px', borderRadius: '12px', fontSize: '12px' }}>
