...
export default async function AdminUsersPage({
  ...
  const ROLE_COLOR: Record<string, string> = {
    CUSTOMER: 'bg-[var(--bg-card)] text-[#888] border border-[#333]',
  ...
  return (
    <div className="bg-[var(--bg-primary)] min-h-full">
      <div className="mb-6 flex items-center justify-between">
        <div>
          <h1 className="text-2xl font-semibold text-[var(--text-primary)]" style={{ fontFamily: 'Georgia,serif' }}>Users</h1>
      ...
      <form className="bg-[var(--bg-secondary)] border border-[var(--bg-card)] rounded-xl p-4 mb-6 flex flex-wrap gap-3">
        ...
        <select name="role" defaultValue={searchParams.role ?? ''} className="text-sm bg-[var(--bg-card)] border border-[#333] text-[var(--text-primary)] rounded-lg px-3 py-2 focus:outline-none focus:ring-2 focus:ring-[#C9A84C]">
          <option value="">All Roles</option>
          <option value="CUSTOMER">Customer</option>
          <option value="VENDOR">Vendor</option>
        </select>
        <select name="status" defaultValue={searchParams.status ?? ''} className="text-sm bg-[var(--bg-card)] border border-[#333] text-[var(--text-primary)] rounded-lg px-3 py-2 focus:outline-none focus:ring-2 focus:ring-[#C9A84C]">
          <option value="">All Status</option>
          <option value="ACTIVE">Active</option>
          <option value="BANNED">Banned</option>
          <option value="PENDING">Pending</option>
        </select>
        <button type="submit" className="px-4 py-2 bg-[#C9A84C] hover:bg-[#F0C040] text-black font-semibold text-sm rounded-lg transition-colors">Filter</button>
      </form>
            ...
            <tbody>
              {users.length === 0 ? (
                ...
                <tr key={user.id} className="border-b border-[var(--bg-card)] hover:bg-[var(--bg-primary)] transition-colors">
                  ...
                  <td className="px-5 py-3">
                    <span className={`inline-flex px-2 py-0.5 rounded-full text-xs font-medium ${ROLE_COLOR[user.role] ?? 'bg-[var(--bg-card)] text-[#888]'}`}>
                      {user.role}
                    </span>
                  </td>
                  <td className="px-5 py-3">
                    <span className={`inline-flex px-2 py-0.5 rounded-full text-xs font-medium ${STATUS_COLOR[user.status] ?? 'bg-[var(--bg-card)] text-[#888]'}`}>
                      {user.status}
                    </span>
                  </td>
                  <td className="px-5 py-3">
