    ...
    <div className="max-w-3xl mx-auto px-4 sm:px-6 lg:px-8 py-12 space-y-10">
      ...
      <div className="grid sm:grid-cols-3 gap-4">
        {[
          ...
          <div key={item.label} className="bg-[var(--bg-secondary)] border border-[#C9A84C]/20 rounded-2xl p-5 text-center">
      ...
      <div>
        <h2 className="text-xl font-bold text-[var(--text-primary)] mb-4">How Commission Works</h2>
        <div className="bg-[var(--bg-secondary)] border border-[#C9A84C]/15 rounded-2xl overflow-hidden">
          <table className="w-full text-sm">
            ...
            <tbody>
              {[
                ...
                <tr key={row.sale} className={`border-b border-[#C9A84C]/10 ${i % 2 === 0 ? '' : 'bg-[var(--bg-primary)]/40'}`}>
                  <td className="px-5 py-3 text-[var(--text-secondary)]">{row.sale}</td>
                  <td className="px-5 py-3 text-right font-semibold text-[var(--text-primary)]">{row.vendor}</td>
                </tr>
              ))}
            </tbody>
          </table>
        </div>
        <p className="text-xs text-[var(--text-secondary)] mt-2 px-1">Commission is deducted before payout. Delivery fees (if applicable) are separate and go to the courier.</p>
      </div>
