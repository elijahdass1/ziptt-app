...
export default async function VendorDigitalPage() {
  ...
  return (
    <div style={{ background: 'var(--bg-primary)', minHeight: '100vh', color: 'var(--text-primary)', padding: '32px' }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '32px' }}>
        <div>
          <h1 style={{ fontFamily: 'Georgia, serif', fontSize: '28px' }}>Digital Products</h1>
          <p style={{ color: 'var(--text-secondary)', marginTop: '4px' }}>Revenue: ${(totalRevenue._sum.vendorEarnings ?? 0).toFixed(2)} TTD (after 15% commission)</p>
        </div>
        <Link href="/vendor/digital/new" style={{ background: '#C9A84C', color: 'var(--bg-primary)', padding: '10px 24px', borderRadius: '8px', textDecoration: 'none', fontWeight: 'bold', fontSize: '14px' }}>
          + Add Digital Product
        </Link>
      </div>
      {products.length === 0 ? (
            ...
            <thead>
              <tr style={{ borderBottom: '1px solid #1A1A1A' }}>
                {['Product', 'Category', 'Price', 'Available', 'Sold', 'Status'].map(h => (
                  <th key={h} style={{ padding: '12px 16px', textAlign: 'left', fontSize: '12px', color: 'var(--text-secondary)', textTransform: 'uppercase', letterSpacing: '1px' }}>{h}</th>
                ))}
              </tr>
            </thead>
            <tbody>
              {products.map(p => (
                <tr key={p.id} style={{ borderBottom: '1px solid #1A1A1A' }}>
                  ...
                  <td style={{ padding: '16px', color: 'var(--text-secondary)', fontSize: '13px' }}>{p.categoryTag}</td>
                  ...
                  <td style={{ padding: '16px' }}>
                    <span style={{ background: p.isActive ? '#052e16' : '#1c1917', color: p.isActive ? '#4ade80' : 'var(--text-secondary)', padding: '3px 10px', borderRadius: '12px', fontSize: '12px' }}>
