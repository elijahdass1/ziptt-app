...
export function VendorMarquee({ vendors }: Props) {
  ...
  return (
    <section className="relative overflow-hidden border-y border-[#C9A84C]/10 bg-[var(--bg-primary)]">
      <div className="absolute inset-y-0 left-0 w-16 bg-gradient-to-r from-[var(--bg-primary)] to-transparent z-10 pointer-events-none" />
      ...
      <div className="ziptt-marquee-track py-4" style={{ animationDuration: '60s' }}>
        {doubled.map((v, i) => (
          <Link
            key={`${v.slug}-${i}`}
            href={`/store/${v.slug}`}
            className="flex items-center gap-2 mx-3 px-4 py-2 rounded-full bg-[var(--bg-secondary)] border border-[#C9A84C]/15 hover:border-[#C9A84C]/45 hover:bg-[var(--bg-card)] transition-colors group shrink-0"
          >
            {v.logo ? (
            ...
            <span className="text-xs font-semibold text-[var(--text-primary)] group-hover:text-[#C9A84C] transition-colors whitespace-nowrap">
              {v.storeName}
            </span>
          </Link>
        ))}
