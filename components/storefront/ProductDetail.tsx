    ...
    <div>
      ...
      <div className="grid md:grid-cols-2 gap-8 lg:gap-12">
        ...
        <div className="space-y-3">
          <div className="aspect-square rounded-2xl overflow-hidden bg-[var(--bg-card)] border border-[#C9A84C]/10">
            <img
        ...
        <div className="space-y-4">
          <div>
            ...
            <h1 className="text-2xl font-bold text-[var(--text-primary)] mt-1">{product.name}</h1>
          </div>
          ...
          {product.reviewCount > 0 ? (
            <div className="flex items-center gap-3">
              ...
              <span className="text-sm text-[var(--text-primary)] font-medium">{avgRating.toFixed(1)}</span>
              <span className="text-sm text-[var(--text-secondary)]">({product.reviewCount} reviews)</span>
              {product.soldCount > 0 && (
                <span className="text-sm text-[var(--text-secondary)]">• {product.soldCount} sold</span>
              )}
            </div>
          ) : (
            <p className="text-sm text-[var(--text-secondary)] italic">No reviews yet — be the first</p>
          )}

          {/* Price */}
          <div className="flex items-baseline gap-3">
            <span className="text-3xl font-black text-[var(--text-primary)]">{formatTTD(product.price)}</span>
            {product.comparePrice && (
              <>
                <span className="text-lg text-[var(--text-secondary)] line-through">{formatTTD(product.comparePrice)}</span>
          ...
          <div className="flex gap-3">
            <button
              ...
              className="flex-1 flex items-center justify-center gap-2 py-3 bg-[#C9A84C] hover:bg-[#F0C040] disabled:bg-[var(--bg-card)] disabled:cursor-not-allowed text-black disabled:text-[var(--text-secondary)] font-bold rounded-xl transition-colors"
            >
          ...
          {/* Delivery info */}
          <div className="bg-[var(--bg-secondary)] border border-[#C9A84C]/15 rounded-xl p-4 space-y-3">
            <div className="flex items-start gap-3">
              <Truck className="h-5 w-5 text-[#C9A84C] shrink-0 mt-0.5" />
              <div>
                <p className="text-sm font-semibold text-[var(--text-primary)]">Delivery</p>
            ...
            <div className="flex items-start gap-3">
              ...
              <div>
                <p className="text-sm font-semibold text-[var(--text-primary)]">Buyer Protection</p>
                <p className="text-xs text-[var(--text-secondary)]">7-day return policy if item is not as described.</p>
              </div>
            </div>
          </div>
          ...
          {tags.length > 0 && (
            <div className="flex flex-wrap gap-2">
              {tags.map((tag: string) => (
                <span key={tag} className="text-xs bg-[var(--bg-card)] border border-[#C9A84C]/15 text-[var(--text-secondary)] px-2.5 py-1 rounded-full">{tag}</span>
              ))}
            </div>
          )}
          {product.sku && <p className="text-xs text-[var(--text-secondary)]/60">SKU: {product.sku}</p>}
        </div>
      </div>
      ...
      <div className="mt-10">
          ...
          <div className="flex gap-6">
            {(['desc', 'reviews'] as const).map((t) => (
              <button key={t} onClick={() => setTab(t)}
                className={`pb-3 text-sm font-semibold border-b-2 transition-colors ${tab === t ? 'border-[#C9A84C] text-[#C9A84C]' : 'border-transparent text-[var(--text-secondary)] hover:text-[var(--text-primary)]'}`}>
        ...
        <div className="py-6">
          {tab === 'desc' ? (
            <div className="text-sm text-[var(--text-secondary)] leading-relaxed whitespace-pre-line">{product.description}</div>
          ) : (
            <div className="space-y-4">
              {product.reviews.length === 0 ? (
                ...
                product.reviews.map((review) => (
                  <div key={review.id} className="bg-[var(--bg-secondary)] border border-[#C9A84C]/15 rounded-xl p-4">
                    <div className="flex items-start gap-3">
                      ...
                      <div className="flex-1 min-w-0">
                        <div className="flex items-center gap-2">
                          <span className="text-sm font-semibold text-[var(--text-primary)]">{review.user.name ?? 'Anonymous'}</span>
                        ...
                        {review.title && <p className="text-sm font-medium text-[var(--text-primary)] mt-2">{review.title}</p>}
                        {review.body && <p className="text-sm text-[var(--text-secondary)] mt-1">{review.body}</p>}
                      </div>
                    </div>
                  </div>
                ))
